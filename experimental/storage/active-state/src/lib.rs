...
// Use for storing evicted items

#[derive(Clone, Debug, Eq, PartialEq, Hash, Copy)]
pub struct StateKeyHash(HashValue);
...
impl ActiveState {
    ...
    fn generate_actions(&mut self, value_set: Vec<(StateKey, Option<StateValue>)>) -> Vec<Action> {
        ...
        for (key, value) in value_set {
            ...
            let state_key_hash = StateKeyHash(key.hash());
            let item_id_opt = self.existing_items.get(&state_key_hash);
    ...
    fn add_items_to_active_state(
        ...
        pairs
            ...
            .for_each(|(key, value)| {
                ...
                self.existing_items.insert(state_key_hash, empty_slot);
        ...
        for action in updates {
            match action {
                ...
                Action::EvictAndAdd(key, value) => {
                    ...
                    self.evict_and_add_item(key, value)?;
                },
                Action::Evict(key) => {
                    self.evict_from_smt(&key)?;
                    self.evict_item(key)?;
    ...
    fn evict_item(&mut self, key: StateKey) -> Result<()> {
        let state_key_hash = StateKeyHash(key.hash());
        if let Some(item_id) = self.existing_items.get(&state_key_hash) {
            ...
            self.existing_items.remove(&state_key_hash);
            self.used_slots_cnt -= 1;
            Ok(())
        } else {
    ...
    fn evict_and_add_item(&mut self, key: StateKey, value: StateValue) -> Result<()> {
        // evict the oldest item
        let new_state_key_hash = StateKeyHash(key.hash());
        if let Some(oldest_item) = self.oldest_item {
            ...
            let state_key_hash = item.inner.as_ref().expect("Oldest item has no inner").key;
            self.oldest_item = item.next;
            self.existing_items.remove(&state_key_hash);

            // use the slot for the new key value pair
            item.inner = Some(ItemInner {
                key: new_state_key_hash,
            ...
            self.existing_items.insert(new_state_key_hash, item.id);
            Ok(())
        } else {
